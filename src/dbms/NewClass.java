...
public class NewClass {
    ...
    public NewClass() throws IOException{
        ...
        System.out.println("Final Average F-measure: " + df.format((2*avPre*avRe/(avPre+avRe))/5) + "%");
    ...
    public void accuracy2() throws IOException{
        ...
        for(Map.Entry m:map.entrySet()){
            ...
            //System.out.println(TP[f]+" "+TN[f]+ " "+FP[f]+ " " +FN[f]);
             
            accuracy[f] = (TP[f]+TN[f])*1.0/(TP[f]+TN[f]+FN[f]+FP[f]) ;
            Precission[f] = TP[f]*1.0 / (TP[f] + FP[f]) ;
            Recall[f] = TP[f]*1.0 / (TP[f] + FN[f]) ;
